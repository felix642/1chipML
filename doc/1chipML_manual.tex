...
The algorithm is implemented in a function that takes in two separate arrays to represent complex values, where the first part consists of real numbers and where the second, imaginary numbers. This algorithm overrides the arrays passed as arguments and places in the result of the FFT. Furthermore, the radix-2 decimation-in-time (DIT) method was used, meaning the arrays must have a length equal to a power of 2. If this is not the case, an error code (0) will be returned by the function. 
...
Given the context of microcontrolers and their lack of memory, multiple design choices were made, which can be summarized here:
\begin{itemize}
	...
	\item The FFT returns 0 in case of an error, 1 otherwise
